...
#define	checkGeometryOrDiscard2()									\
	if (attributes->flags & ATTRIBUTES_FLAGS_DISCARD_GEOMETRY)		\
		return true;
...
void	CRendererContext::RiAbcArchiveV(const char* name, int n,const char *tokens[],const void *params[]){
#ifdef USE_ALEMBIC
	std::ostringstream args;
	args << "-filename " << std::string(name);
	for(; n--;){
		CVariable	*cVar;
		CVariable	tVar;
		...
		cVar = CRenderer::retrieveVariable(tokens[n]);

		if (cVar == NULL){
			if (parseVariable(&tVar,NULL,tokens[n]) == TRUE) {
				cVar	=	&tVar;
			}
		}

		if (!cVar){
			error(CODE_BADTOKEN,"Bad token for RiAbcArchive : '%s'\n", tokens[n]);
			continue;
		}
		...
		switch(cVar->type){
			case (TYPE_STRING):
				parameter_char = ((char**)params[n])[0];
				if (strcmp(name, "objectpath") == 0)
					args << " -objectpath " << std::string(parameter_char);
				break;
			case (TYPE_FLOAT):
				parameter_float = ((float*)params[n])[0];
				if (strcmp(name, "shutteropen") == 0)
					args << " -shutteropen " << parameter_float;
				if (strcmp(name, "shutterclose") == 0)
					args << " -shutterclose " << parameter_float;
				break;
			case (TYPE_INTEGER):
				parameter_int = (int)((float*)params[n])[0];
				if (strcmp(name, "frame") == 0)
					args << " -frame " << parameter_int;
				if (strcmp(name, "fps") == 0)
					args << " -fps " << parameter_int;
				if (strcmp(name, "flipv") == 0 && parameter_int == 1)
					args << " -flipv ";
				break;
			default:
				error(CODE_BADTOKEN,"Bad token for RiAbcArchive : '%s'\n", name);
	...
	importAbcArchive(args.str().c_str());
#else
	error(CODE_INCAPABLE,"No Alembic support\n");
#endif
}
...
void	CRendererContext::RiClipping(float hither,float yon) {
	...
	options				=	getOptions(TRUE);
	options->clipMin	=	maxx(hither,C_EPSILON);
	options->clipMax	=	yon;
...
void	CRendererContext::RiCustomDisplayV(const char *name, RtToken mode, RtDisplayStartFunction startFunction, RtDisplayDataFunction dataFunction, RtDisplayFinishFunction finishFunction, RtInt n, RtToken tokens[], RtPointer params[]) {
	...
	cDisplay->cppDisplayWrap		    =	NULL;
	options->displays				=	cDisplay;
}

void	CRendererContext::RiCustomDisplay2V(const char *name, RtToken mode, void* cppDisplayWrap, int n,const char *tokens[],const void *params[]) {
	COptions	*options			=	getOptions(TRUE);
	COptions::CDisplay	*cDisplay	=	new COptions::CDisplay;
	if ( strcmp( name, "dtex" ) )
        cDisplay->outDevice				=	strdup("custom2");
    else
        cDisplay->outDevice				=	strdup("customdtex");
	cDisplay->outName				=	strdup(name);
	cDisplay->outSamples			=	strdup(mode);
	cDisplay->next					=	options->displays;
	cDisplay->startFunction			=	NULL;
	cDisplay->dataFunction			=	NULL;
	cDisplay->finishFunction		=	NULL;
	cDisplay->cppDisplayWrap        =   cppDisplayWrap;
	options->displays				=	cDisplay;
}
	...
	if (attributes != NULL) {
		...
		// Check the irradiance cache options
			...
			for (i=0;i<n;i++) {
				...
				attributeCheckFlag(RI_TRACE,            attributes->flags,  ATTRIBUTES_FLAGS_SPECULAR_VISIBLE|ATTRIBUTES_FLAGS_DIFFUSE_VISIBLE)
					warning(CODE_BADTOKEN,"Visibility attribute \"trace\" is deprecated and will be interpreted as both \"specular\" and \"diffuse\"\n");
				attributeEndCheck
			}
...
void	CRendererContext::RiPointsGeneralPolygonsV(int npolys,int *nloops,int *nverts,int *verts,int n,const char *tokens[],const void *params[]) {
	...
	for (i=0;i<sverts;i++) {
		numVertices	=	maxx(numVertices,verts[i]);
	}
...
void	CRendererContext::RiSphereV(float radius,float zmin,float zmax,float thetamax,int n,const char *tokens[],const void *params[]) {
	...
	case 1:
		...
		if (	(p0[0] != 0)		&&
			...
			tmp			=	absf(p0[0]);
			p0[1]		=	minn(maxx(p0[1],-tmp),tmp);
			p0[2]		=	minn(maxx(p0[2],-tmp),tmp);
	...
	case 2:
		...
		if (	((p0[0] != 0)		||	(p1[0] != 0))		&&
			...
			tmp			=	absf(p0[1]);
			p1[1]		=	minn(maxx(p1[1],-tmp),tmp);
			p1[2]		=	minn(maxx(p1[2],-tmp),tmp);
...
void	CRendererContext::RiMakeSphericalEnvironmentV(const char *pic,const char *tex,float (*filterfunc)(float,float,float,float),float swidth,float twidth,int n,const char *tokens[],const void *params[]) {
	COptions	*options;

	if (CRenderer::netClient != INVALID_SOCKET)	return;

	options	=	getOptions(FALSE);
	makeSphericalEnvironment(pic,tex,options->texturePath,RI_PERIODIC,RI_CLAMP,filterfunc,swidth,twidth,n,tokens,params);
}
...
void extractColor(const char* param,color& col){
    char *colg,*colb,*colr;

    colr = strtok((char*)param," ");
    if(!colr) return;
...
void CRendererContext::RiPtFilterV(const char *filterType, const char* filename, const char* outfilename, int n, const char *tokens[], const void* params[]){
/*    color albedo,dmfp;
    float maxsolidangle,ior,unitlength;
    ...
    if (strcmp(filterType,"sss") == 0){
        for(int i = 0; i < n;i++){
            ...
            float* param = (float*)params[i];

            if( strcmp(tokens[i],"albedo") == 0){
                albedo.r = param[0];
                albedo.g = param[1];
                albedo.b = param[2];
            }
            if( strcmp(tokens[i],"dmfp") == 0 ){
                dmfp.r = param[0];
                dmfp.g = param[1];
                dmfp.b = param[2];
            }
            if( strcmp(tokens[i],"ior") == 0 ){
                ior = param[0];
            }
            if( strcmp(tokens[i],"maxsolidangle") == 0 ){
                maxsolidangle = param[0];
            }
            if( strcmp(tokens[i],"unitlength") == 0 ){
                unitlength = param[0];
            }
            if( strcmp(tokens[i],"progress") == 0 ){
                progress = ((int*)params[i])[0];
            }

        }

        SSDiffusion((char**)&filename, (char*)outfilename, 1,0,&albedo,0,&dmfp,unitlength, ior, maxsolidangle,progress);
        ...
        GIDiffusion(( char**)&filename, (char*)outfilename,1,maxsolidangle,progress);

    } else {
        error(CODE_BADTOKEN,"Invalid filter type");
    }
*/
}
