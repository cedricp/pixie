...
#define SATURATEEXP float Q = sqrt( op1[0]*op1[0]*.241 + op1[1]*op1[1]*.691 + op1[2]*op1[2]*.068 ); \
                    res[0] = Q + (op1[0] - Q) *  (*op2); \
                    res[1] = Q + (op1[1] - Q) *  (*op2); \
                    res[2] = Q + (op1[2] - Q) *  (*op2); \

DEFFUNC(Saturatec       ,"saturate"     ,   "c=cf"  , FUN3EXPR_PRE, SATURATEEXP,FUN3EXPR_UPDATE(3,3,1),NULL_EXPR,0)
...
#define MATCHEXPR			if (strstr(*op2,*op1) != 0)								\
								*res	=	1;										\
							else													\
								*res	=	0;
