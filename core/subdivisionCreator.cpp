...
public:
	...
	void			split() {
						if (childVertex == NULL) {
							...
							childVertex->sharpness		=	maxx(sharpness-1,0);
						}
					}
...
public:
	...
	void			split() {
						if (childVertex == NULL) {
							...
							children[0]->sharpness		=	maxx(sharpness-1,0);

							children[1]->vertices[0]	=	vertices[1]->childVertex;
							children[1]->vertices[1]	=	childVertex;
							children[1]->sharpness		=	maxx(sharpness-1,0);
						...
						if ((split == FALSE) || (funny == TRUE)) {
							...
							if (funny == FALSE) {
								...
								if ( funnyBorder == TRUE ) {
									...
									if ( (numExtraordinary > 0) && (vertices[(extraordinary+0)&3]->valence >= 3)) {
										// We have an extraordinary patch
										// divide main patch into two side strips which are adjacent
										// to the extraordinary patch and a smaller grid
...
CSubdivMesh::~CSubdivMesh() {
	...
	if (varyingData)     free(varyingData);
	if (faceVaryingData) free(faceVaryingData);
...
void		CSubdivMesh::create(CShadingContext *context) {
	...
	// use malloc instead of ralloc here, else it causes a bug in s-t mapping, memory released too early
	// Make sure it keeps varying datas

	pl->collect(data.vertexSize,data.vertexData,CONTAINER_VERTEX,context->threadMemory,true);
    pl->collect(data.varyingSize,data.varyingData,CONTAINER_VARYING,context->threadMemory,true);
    pl->collect(data.facevaryingSize,data.facevaryingData,CONTAINER_FACEVARYING,context->threadMemory,true);

    vertexData      = data.vertexData;
    varyingData     = data.varyingData;
    faceVaryingData = data.facevaryingData;
	...
	for (i=0,cnargs=nargs,cintargs=intargs,cfloatargs=floatargs;i<ntags;i++) {
		if (strcmp(tags[i],RI_HOLE) == 0) {
			...
			for (j=0;j<cnargs[0]-1;j++) {
				...
				if (cEdge != NULL) {
					cEdge->sharpness	=	minn(cfloatargs[0],10);
				} else {
        ...
        for (j=0;j<numEdges;j++) {
            ...
            if (cEdge->faces[1] == NULL) {
                ...
                if (data.currentFlags & FACE_INTEPOLATEBOUNDARY){
                    ...
                    /*CSFace* boundaryFace = new (data.context) CSFace(data,numFaces++);
                    boundaryFace->numEdges		=	4;
                    boundaryFace->vertices		=	(CSVertex **)	ralloc(4*sizeof(CSVertex *) ,data.context->threadMemory);
                    boundaryFace->edges		    =	(CSEdge **)		ralloc(4*sizeof(CSEdge *)   ,data.context->threadMemory);

                    CSEdge* cEdge0				=	new (data.context) CSEdge(data);
                    CSEdge* cEdge1				=	new (data.context) CSEdge(data);
                    CSEdge* cEdge2				=	new (data.context) CSEdge(data);
                    CSEdge* cEdge3				=	new (data.context) CSEdge(data);

                    boundaryFace->edges[0] = cEdge0;
                    boundaryFace->edges[1] = cEdge1;
                    boundaryFace->edges[2] = cEdge2;
                    boundaryFace->edges[3] = cEdge3;

                    boundaryFace->vertices[0]   =   cEdge->vertices[0];
                    boundaryFace->vertices[1]   =   cEdge->vertices[1];
                    boundaryFace->vertices[2]   =   cEdge->vertices[1];
                    boundaryFace->vertices[3]   =   cEdge->vertices[0];

                    cEdge0->vertices[0] = boundaryFace->vertices[0];
                    cEdge0->vertices[1] = boundaryFace->vertices[1];
                    cEdge1->vertices[0] = boundaryFace->vertices[1];
                    cEdge1->vertices[1] = boundaryFace->vertices[2];
                    cEdge2->vertices[0] = boundaryFace->vertices[2];
                    cEdge2->vertices[1] = boundaryFace->vertices[3];
                    cEdge3->vertices[0] = boundaryFace->vertices[3];
                    cEdge3->vertices[1] = boundaryFace->vertices[0];

                    cEdge0->vertices[1]->addFace(boundaryFace);
                    cEdge1->vertices[1]->addFace(boundaryFace);
                    cEdge2->vertices[1]->addFace(boundaryFace);
                    cEdge3->vertices[1]->addFace(boundaryFace);

                    cEdge0->vertices[0]->addFace(boundaryFace);
                    cEdge1->vertices[0]->addFace(boundaryFace);
                    cEdge2->vertices[0]->addFace(boundaryFace);
                    cEdge3->vertices[0]->addFace(boundaryFace);

                    cEdge0->vertices[0]->addEdge(cEdge0);
                    cEdge0->vertices[1]->addEdge(cEdge0);
                    cEdge1->vertices[0]->addEdge(cEdge1);
                    cEdge1->vertices[1]->addEdge(cEdge1);
                    cEdge2->vertices[0]->addEdge(cEdge2);
                    cEdge2->vertices[1]->addEdge(cEdge2);
                    cEdge3->vertices[0]->addEdge(cEdge3);
                    cEdge3->vertices[1]->addEdge(cEdge3);

                    cEdge->vertices[0]->addFace(boundaryFace);
                    cEdge->vertices[1]->addFace(boundaryFace);
                    cEdge0->vertices[0]->addFace(cFace);
                    cEdge0->vertices[1]->addFace(cFace);

                    cEdge1->vertices[0]->addFace(cFace);
                    cEdge3->vertices[1]->addFace(cFace);

                    cEdge0->addFace(cFace);
                    cEdge0->addFace(boundaryFace);
                    cEdge1->addFace(boundaryFace);
                    cEdge2->addFace(boundaryFace);
                    cEdge3->addFace(boundaryFace);

                    cEdge->vertices[1]->addFace(boundaryFace);
                    cEdge->vertices[0]->addFace(boundaryFace);

                    cEdge->addFace(boundaryFace);*/
                }
            }
        }
	}
