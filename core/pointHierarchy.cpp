		...
		if (currentNode < 0) {
			...
			//if ((node->dN > 0.999999) && (dotvv(P,node->N) <= dotvv(node->P,node->N))) {
                ...
                if (	( lenD > node->dP) && ((dParea / distSq) < maxsolidangle) 	) {
                    const float form = ff(P,N,node->P,node->N,node->dP,coneAngle);

                    if (radiosityIndex > 0) {
                        float fallOffCoeff = pow(lenD,-fallOff);

                        Cl[0] += form*(node->radiosity[0] * fallOffCoeff);
                        Cl[1] += form*(node->radiosity[1] * fallOffCoeff);
                        Cl[2] += form*(node->radiosity[2] * fallOffCoeff);
                    }
                    Cl[3]	+=	form;
                } else {
                    // Sanity check
