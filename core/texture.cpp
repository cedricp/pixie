...
static inline unsigned char	*textureAllocateBlock(CTextureBlock *entry,CShadingContext *context) {
	...
	stats.peakTextureSize							=	maxx(stats.textureSize,stats.peakTextureSize);
...
static inline void	textureLoadBlock(CTextureBlock *entry,char *name,int x,int y,int w,int h,int dir,CShadingContext *context) {
	...
	if (in != NULL) {		// Error, we opened this file before
		...
		// DWARF LABS : Or maybe is it PNG file ?
		png_structp png_ptr;
		png_infop info_ptr;
		png_uint_32 width, height,row;
		int bit_depth, color_type, interlace_type;
		int filter_method,compression_type;
		FILE *in;
			...
			if (png_ptr){
				...
				if (info_ptr){
					...
					// Not yet !
						...
						png_bytepp row_pointers = (png_bytepp)malloc(height*sizeof(png_bytepp));
						int size = png_get_rowbytes(png_ptr,info_ptr);

						for (row = 0; row < height; row++)
							row_pointers[row] = (png_bytep)png_malloc(png_ptr, size);

						png_read_image(png_ptr, row_pointers);

						unsigned char* cData = (unsigned char*)data;
						for (row = 0; row < height; row++){
							memcpy(cData,row_pointers[row],size);
							cData += size;
						}
...
public:
    ...
    float               lookupZ(float s,float t,CShadingContext *context) {
							s		*=	width;						// To the pixel space
							t		*=	height;
							s		-=	0.5;						// Pixel centers
							t		-=	0.5;
							...
							return (res[0] * (1-ds)*(1-dt))  + (res[3] * ds*(1-dt) )  + ( res[6] * (1-ds)*dt) + ( res[9] * ds*dt);
						}
...
public:
	...
	float				lookupZ(float s,float t,CShadingContext *context) {
							assert(numLayers > 0);
							return layers[0]->lookupZ(s,t,context);
						}
...
public:
	...
	float				lookupZ(float s,float t,CShadingContext *context) {
							assert(layer != NULL);
							return layer->lookupZ(s,t,context);
						}
...
public:
	...
	virtual				~CShadow() {
                            if (sampleCoordX) free(sampleCoordX);
                            if (sampleCoordY) free(sampleCoordY);

							if (side != NULL)	delete side;
						}

	void                hammersleyToRadial( bool shadowSampling, float& sampleCoordX,float& sampleCoordY ){
                            // if these samples are for light shadowing sampling
                            if ( shadowSampling == true ){
                                // samples should be evenly distributed
                                sampleCoordY = sqrtf( sampleCoordY );

                                // vary the samples positions per shaded point
                                sampleCoordX = fmod( sampleCoordX + randomNumber, 1.0f );
                            }

                            float angleOfSample = sampleCoordX * 2 * M_PI;
                            sampleCoordX = cos( angleOfSample ) * sampleCoordY; // cos = adj / hypot
                            sampleCoordY = sqrtf( (sampleCoordY * sampleCoordY) - (sampleCoordX * sampleCoordX)  ); // a2 + b2 = c2
                            if( angleOfSample > M_PI )
                                sampleCoordY *= -1;
                        }

    float               sampleShadowMap( float pShadS, float pShadT, float sampleX, float sampleY, float radius, bool us, float& distFromPs, CShadingContext *context){
                            if ( pShadS < 0.0 || pShadS > 1.0 || pShadT < 0.0 || pShadT > 1.0)
                                return FLT_MAX;

                            float ss = sampleX;
                            float tt = sampleY;

                            hammersleyToRadial( us, ss, tt );
                            ss *= radius;
                            tt *= radius;
                            ss += pShadS;
                            tt += pShadT;

                            float slen = (pShadS - ss);
                            float tlen = (pShadT - tt);

                            distFromPs = sqrtf(  slen*slen + tlen*tlen );

                            if ( ss < 0.0 || ss > 1.0 || tt < 0.0 || tt > 1.0)
                                return FLT_MAX;

                            return side->lookupZ(ss,tt,context);
                        }


    void                initSamples(int samples){
                            float log2 = log(2.0f);
                            if ( hammersleySamples == samples )
                                return;

                            if (sampleCoordX) free(sampleCoordX);
                            if (sampleCoordY) free(sampleCoordY);

                            sampleCoordX = (float*)malloc( sizeof(float) * samples);
                            sampleCoordY = (float*)malloc( sizeof(float) * samples);

                            hammersleySamples = samples;

                            // find the num of decimals
                            int numOfDigits = ceil( log( (double)samples) / log2 );
                            float* revBinValues = (float*)alloca( sizeof(float) * (float)numOfDigits );
                            ...
                            for( int iter = 0; iter < samples; ++iter )
                            {
                                ...
                                for( int digit = (numOfDigits - 1); digit >= 0; --digit )
                                {
                                    float power = powf( 2, (float)digit );
                                    if ( remainder >= power  )
                                    {
                                ...
                                {
                                    x += ( powf( 2.0, -(float)index ) * revBinValues[ numOfDigits-index ] );
                                    y += ( powf( 2.0, -(float)index ) * revBinValues[ index-1 ] );
                                }

                                // scale x to compensate for the non-power-of-two sample numbers
                                x *= powf( 2, (float)numOfDigits ) / (float)samples ;
	...
	void				lookup(float *result,const float *D0,const float *D1,const float *D2,const float *D3,CShadingContext *context) {
                            ...
                            float	tmp[4],cP[4];
                            float   regularblur  = scratch->textureParams.blur;
                            float   avrgedDepth  = 0;
                            float   minradius    = (float)scratch->traceParams.minradius;
                            float   maxradius    = (float)scratch->traceParams.maxradius;
                            bool    softshadow   = scratch->textureParams.soft > 0.0;
                            const int numSamples = (int)scratch->traceParams.samples;
                            const float biasCone = (int)scratch->traceParams.biasconeslope;


							if (softshadow){
                                // Code found at http://www.3db-site.com/html/softShadow.php
                                // From Disney's softshadow paper
                                initSamples(numSamples);

                                float totalweight  = 0;
                                float weight       = 0;
                                float shadowValue  = 0;

                                cP[COMP_X]			=	(D0[COMP_X]*(.5) + D1[COMP_X]*.5)*(.5) + (D2[COMP_X]*(.5) + D3[COMP_X]*.5)*.5;
                                cP[COMP_Y]			=	(D0[COMP_Y]*(.5) + D1[COMP_Y]*.5)*(.5) + (D2[COMP_Y]*(.5) + D3[COMP_Y]*.5)*.5;
                                cP[COMP_Z]			=	(D0[COMP_Z]*(.5) + D1[COMP_Z]*.5)*(.5) + (D2[COMP_Z]*(.5) + D3[COMP_Z]*.5)*.5;
                                cP[3]				=	1;
                                ...
                                pShadS = tmp[COMP_X]/tmp[3];
                                pShadT = tmp[COMP_Y]/tmp[3];

                                // get Ps distance from shadow plane
                                mulmp4(tmp,toCamera,cP);
                                float PsDepth = tmp[COMP_Z]/tmp[3];
                                ...
                                for (int j = 0; j < numSamples ; j++){
                                    ...
                                    sampleDepth = sampleShadowMap( pShadS, pShadT, sampleCoordX[j], sampleCoordY[j], maxradius, false, distFromPs, context );


                                    if ( (sampleDepth + scratch->traceParams.bias) < PsDepth )
                                    {
                                ...
                                if ( avrgedDepth > 0 ) {
                                    ...
                                    for (int j = 0; j < numSamples ; j++){
                                        float sampleDepth, distFromPs;
                                        randomNumber = context->urand();
                                        sampleDepth = sampleShadowMap( pShadS, pShadT, sampleCoordX[ j ], sampleCoordY[ j ], filterRadius, true, distFromPs, context );

                                        // add bias cone
                                        sampleDepth += scratch->traceParams.bias + ( distFromPs * biasCone );
                                    ...
                                    shadowValue /= scratch->traceParams.samples;
                                ...
                                if ( scratch->traceParams.maxDist > 0)
                                    shadowValue *= 1 - ( smoothstep( 0, scratch->traceParams.maxDist, distFromOccluder )  );

                                result[0] = shadowValue;
                                result[1] = result[0];
                                result[2] = result[0];
                                return;
							}
                            ...
                            for (int i=(int) scratch->traceParams.samples;i>0;--i) {
                                ...
                                float slen = regularblur*(r[2] - 0.5f);
                                float tlen = regularblur*(r[3] - 0.5f);
                                biascone += sqrt ( (slen*slen) + (tlen*tlen) ) * biasCone;
                                s				+=	slen;
                                t				+=	tlen;
                                ...
                                result[0]			+=	contribution * side->lookupz(s,t,tmp[2]-(scratch->traceParams.bias + biascone),context);
                            }
...
public:
	...
	void				lookup(float *result,const float *D0,const float *D1,const float *D2,const float *D3,CShadingContext *context) {
							...
							for (int i=(int) scratch->traceParams.samples;i>0;--i) {
								...
								float spos					=	tmp[0] / tmp[3];
								float tpos					=	tmp[1] / tmp[3];



                                s				= spos + scratch->textureParams.blur*(r[2] - 0.5f);
                                t				= tpos + scratch->textureParams.blur*(r[3] - 0.5f);
								...
								lookupz(s,t,     &cTile, &cPixel, context);
								...
								while(TRUE) {
                                    if (cPixel[0] > w)	{cPixel -= 4; continue;}
									else if (cPixel[4] < w)  {cPixel += 4; continue;}
                                    else {
        ...
        void            lookupz(int s, int t, CDeepTile **cTile,float **cPixel,CShadingContext *context){
                            int			        bx,by,px,py;
                            ...
                            px					&=  (1 << header.tileShift) - 1;
                            py					&=  (1 << header.tileShift) - 1;
...
template <class T> static CTexture	*readMadeTexture(const char *name,const char *aname,TIFF *in,int &dstart,int width,int height,const char *smode,const char *tmode,T enforcer) {
	...
	for (int i=0;i<pyramidSize;++i) {
		...
		TIFFGetFieldDefaulted(in,TIFFTAG_TILELENGTH             ,&tileHeight);
		int ii, jj;
		for (ii=1,jj=0;ii != (int)tileWidth;ii = ii << 1,jj++);
		tileWidthShift           =       jj;
		for (ii=1,jj=0;ii != (int)tileHeight;ii = ii << 1,jj++);
		tileHeightShift           =       jj;
...
// Comments				:
template <class T> static CTexture	*readTexturePng(const char *name,const char *aname,png_structp *png_s,png_infop *png_i,int width, int height, int bit_depth,int numChannels) {
	double		M;
	if (sizeof(T) == sizeof(float)) {
		M		=	1;
	} else if(sizeof(T) == sizeof(unsigned short)) {
	...
	cTexture->layer				=	new CBasicTexture<T>(name,0,width,height,numChannels,width,height,TEXTURE_BLACK,TEXTURE_BLACK,M);
	return cTexture;
}


///////////////////////////////////////////////////////////////////////
// Function				:	textureLoadPng
// Description			:	Load a texture from disk in PNG format
// Return Value			:	Pointer to the new texture
// Comments				:
static	CTexture	*texLoadPng(const char *name,const char *aname,FILE *in) {
	...
	if (numChannels != 0){
		if (bit_depth == 8)
			cTexture = readTexturePng<unsigned char>(name,aname,&png_ptr,&info_ptr,width,height,bit_depth,numChannels);
		if (bit_depth == 16)
			cTexture = readTexturePng<unsigned short>(name,aname,&png_ptr,&info_ptr,width,height,bit_depth,numChannels);
	}
...
CTexture		*CRenderer::textureLoad(const char *name,TSearchpath *path) {
	...
	if (in != NULL) {
		...
		{
			fread(header, 1, 4, fp);
			is_png = !png_sig_cmp((png_bytep)header, 0, 4);
			if(is_png){
				cTexture		=	texLoadPng(fn,name,fp);
			}
			fclose(fp);
		}
	}
