...
void		CRenderer::beginClipping() {
	...
	const float	minX		=	minn(pixelLeft,pixelRight);	// The extend of the rendering window on the image
	const float	maxX		=	maxx(pixelLeft,pixelRight);	// plane
	const float	minY		=	minn(pixelTop,pixelBottom);
	const float	maxY		=	maxx(pixelTop,pixelBottom);
