...
typedef struct Tstringdef {
	char* string;			// String value, null terminated and escaped
	int length;				// String length
	int token;				// Token representing string
	Tstringdef *next;		// Next string reference
} Tstringdef;

// Internal state of ribreader, tightly coupled to 'ribin'
typedef struct TribreaderState {
	char* expandbuf;		// Internal expand buffer
	int expandidx;			// Current head of intbuf
	int expandsize;			// Size of intbuf, maxlen+padding
	unsigned char* readbuf;	// Read buffer
	int readidx;			// Current head of read buffer
	int readsize;			// Actual bytes in readbuf
	int readbufEOF;			// Flag if EOF reached
	Tstringdef *stringrefs;	// Linked list of string references
	char *ri_request[256];	// 256 RI requests
} TribreaderState;

TribreaderState *ribreaderState;
...
static	int		parameterListCheck() {
	...
	for (i=0;i<numParameters;i++) {
		...
		if (var == NULL) {
            ...
            else if (strcmp(par->name,"Pref") == 0) {
				tmp.numFloats	=	3;
				tmp.numItems	=	1;
				tmp.entry		=	VARIABLE_P;
				strcpy(tmp.name,"Pref");
				tmp.type		=	TYPE_POINT;
				tmp.container	=	CONTAINER_VERTEX;
				tmp.usageMarker	=	PARAMETER_P;
				var				=	&tmp;
...
ribComm:		RIB_STRUCTURE_COMMENT
				...
				ribPL
				{
					RiAbcArchiveV($2,numParameters,tokens,vals);
				}
				|
				RIB_CLIPPING
				RIB_FLOAT
				...
				*/
				...
				ribPL
				{
                    RiPtFilterV($2,$3,$4,numParameters,tokens,vals);
				}
				|
				RIB_MAKE_SHADOW
				RIB_TEXT
...
void	ribParse(const char *fileName,void (*c)(const char *,...)) {
	...
	if (fileName != NULL) {
		...
		TribreaderState		*savedRibreaderState				=	ribreaderState;
...
///////////////////////////////////////////////////////////////////////
// Function				:	ribreadInit
// Description			:	Init function for ribread
// Return Value			:
// Comments				:
void		ribreadInit() {
	ribreaderState		=	new TribreaderState;
	memset(ribreaderState, 0, sizeof(TribreaderState));
}

///////////////////////////////////////////////////////////////////////
// Function				:	ribreadCleanup
// Description			:	Cleanup function for ribread
// Return Value			:
// Comments				:
void		ribreadCleanup() {
	...
	int i;
	Tstringdef *ref = NULL;
	Tstringdef *reftmp = NULL;
	if (ribreaderState->stringrefs) {
		for (ref=ribreaderState->stringrefs;ref;) {
			if (ref->string)
				free(ref->string);
			reftmp=ref->next;
			free(ref);
	...
	for (i=0;i<256;i++)
		if (ribreaderState->ri_request[i])
			free(ribreaderState->ri_request[i]);

	if (!ribreaderState->expandbuf)
		free(ribreaderState->expandbuf);

	if (!ribreaderState->readbuf)
		free(ribreaderState->readbuf);

	delete ribreaderState;
	ribreaderState = NULL;
}

#define EXPANDBUFFER_SIZE 16384			// Initial size of expandbuffer (may be realloc'ed if too small)
#define READBUFFER_SIZE 16384			// Size of read buffer

///////////////////////////////////////////////////////////////////////
// Function				:	ribreadGetByte
// Description			:	Get one byte from readbuffer, filling it if needed
// Return Value			:
// Comments				:	If 'required' flag is not set a fatal error is not generated
//						:	if no bytes are available; readbufEOF should be tested as the value returned is a dummy.
//						:	The readbufEOF flag is always set on EOF
unsigned char	ribreadGetByte(int required=1) {

	if (ribreaderState->readsize - ribreaderState->readidx < 1 ) {
		// Buffer needs filling

		memmove(ribreaderState->readbuf,&ribreaderState->readbuf[ribreaderState->readidx],ribreaderState->readsize-ribreaderState->readidx);
		ribreaderState->readsize -= ribreaderState->readidx;
		ribreaderState->readidx = 0;
		int ret = 0;
#if HAVE_ZLIB
		ret = gzread((gzFile)ribin,&ribreaderState->readbuf[ribreaderState->readsize],READBUFFER_SIZE-ribreaderState->readsize);
#else
		ret = read(ribin,&ribreaderState->readbuf[ribreaderState->readsize],READBUFFER_SIZE-ribreaderState->readsize);
#endif
		if (ret < 0)
		...
		if (ret == 0) {
			// End-of-file
			ribreaderState->readbufEOF = 1;

			if (required == 1)
				fatal(CODE_BADTOKEN,"Premature RIB end of file\n");
			else
				return 0;	// Dummy value
		}

		ribreaderState->readsize += ret;
	}

	return ribreaderState->readbuf[ribreaderState->readidx++];
}

///////////////////////////////////////////////////////////////////////
// Function				:	ribreadRequireExpandbuf
// Description			:	Makes sure expand buffer is of at least 'minbytes' size
// Return Value			:
// Comments				:
void			ribreadRequireExpandbuf(int minbytes) {
	if (ribreaderState->expandidx + minbytes <= ribreaderState->expandsize)
		return;	// Have space

	// If minbytes is not available, one could first return the already expanded
	// data and re-doing the last token.
	// This would require the read buffer to be able to back up to the last expanded token
	// making it more complicated.

	int newsize = ribreaderState->expandsize << 1;	// Try at least doubling to prevent lots of realloc's
	if (ribreaderState->expandidx + minbytes > newsize)
		newsize = ribreaderState->expandidx + minbytes;
	ribreaderState->expandbuf = (char*)realloc(ribreaderState->expandbuf, newsize);
	ribreaderState->expandsize = newsize;

	if (!ribreaderState->expandbuf)
		fatal(CODE_LIMIT,"Failed to allocate buffer for binary RIB\n");
}

///////////////////////////////////////////////////////////////////////
// Function				:	ribread
// Description			:	Alternative function for YY_INPUT which transcodes binary RIBs to ASCII
// Return Value			:	Number of bytes read into 'buf'
// Comments				:
int			ribread(char* buf, size_t maxlen) {

	if (ribreaderState->readbufEOF)
		return 0;

	// Allocate two temporary buffers:
	// expandbuf: a temporary buffer to expand the compressed data into. Cannot use 'buf' directly
	//            because expandbuf needs to be able to fit an entire expanded token which could exceed 'maxlen'.
	// readbuf:   an input buffer to read compressed data into

	if (!ribreaderState->expandbuf) {
		ribreaderState->expandsize = EXPANDBUFFER_SIZE;	// Initial size
		if (ribreaderState->expandsize < maxlen)
			// Expandbuf needs to be atleast maxlen bytes.
			ribreaderState->expandsize = maxlen;
		ribreaderState->expandbuf = (char*)malloc(ribreaderState->expandsize);
		ribreaderState->expandidx = 0;
	} else if (maxlen > ribreaderState->expandsize) {
		// Expandbuf needs to be atleast maxlen bytes.
		// This only happens if 'maxlen' is suddenly larger than before
		ribreaderState->expandbuf = (char*)realloc(ribreaderState->expandbuf, maxlen);
		ribreaderState->expandsize = maxlen;
	}

	if (!ribreaderState->readbuf) {
		ribreaderState->readbuf = (unsigned char*)malloc(READBUFFER_SIZE);
		ribreaderState->readidx = 0;

#if HAVE_ZLIB
		ribreaderState->readsize = gzread((gzFile)ribin,ribreaderState->readbuf,READBUFFER_SIZE);
#else
		ribreaderState->readsize = read(ribin,ribreaderState->readbuf,READBUFFER_SIZE);
#endif
		if (ribreaderState->readsize < 0)
			YY_FATAL_ERROR( "input in flex scanner failed" );
	}
	...
	while(1) {
		...
		if (ribreaderState->expandidx >= maxlen) {
			// Have enough data in expandbuf to fill the output buffer
			memcpy(buf,ribreaderState->expandbuf,maxlen);
			memmove(ribreaderState->expandbuf,&ribreaderState->expandbuf[maxlen],ribreaderState->expandidx-maxlen);
			ribreaderState->expandidx -= maxlen;
			return maxlen;	// Yield
		}

		const int c = ribreadGetByte(0);		// Compression token
		if (ribreaderState->readbufEOF) {
			// End-of-data, last buffer
			memcpy(buf,ribreaderState->expandbuf,ribreaderState->expandidx);
			return ribreaderState->expandidx;	// Yield
		}

		if (c < 128) {
			// Low ASCII, just copy to destination
			ribreaderState->expandbuf[ribreaderState->expandidx++] = c;
			continue;
		}


		// Have high-ASCII == binary RIB; 'c' is a compressed token. (see page 177 of RiSpec 3.2.1)

		if (c >= 0200 && c <= 0217) {
			// Integers and fixed point numbers

			ribreadRequireExpandbuf(20);	// Max size of decimal expansion of a number + padding
			const int d = (c - 0200) >> 2;
			const int w = c - 0200 - (d << 2) + 1;
			signed int val = 0;
			for (j=0; j<w-d; j++)
				if (j == 0)
					val = (signed char)ribreadGetByte();		// Sign-extend
				else
					val = (val << 8) | ribreadGetByte();
			if (d==0) {
				ribreaderState->expandidx += sprintf(&ribreaderState->expandbuf[ribreaderState->expandidx]," %d ",val);
			} else {
				unsigned int val2 = 0;
				for (j=0; j<d; j++)
					val2 = (val2 << 8) | ribreadGetByte();
				float f = (float)val2/(float)(1<<(d*8));
				if (val>0)
					f = val+f;	//  3 + 0.03 =  3.03
				else
					f = val-f;	// -3 - 0.03 = -3.03
				ribreaderState->expandidx += sprintf(&ribreaderState->expandbuf[ribreaderState->expandidx]," %g ",f);
			}
			...
			if (c <= 0237) {
				...
				// Long string
				const int l = c - 0240 + 1;
				len = 0;
				for (j=0; j<l; j++)
					len = (len << 8) | ribreadGetByte();
			}
			ribreadRequireExpandbuf(4*len+3);		// Worst case: all characters need octal escapes
			ribreaderState->expandbuf[ribreaderState->expandidx++] = '"';
			for(j=0; j<len; j++) {
				char c=ribreadGetByte();
					switch(c) {
						case 0x09:
							ribreaderState->expandbuf[ribreaderState->expandidx++] = '\\';
							ribreaderState->expandbuf[ribreaderState->expandidx++] = 't';
							break;
						case 0x0a:
							ribreaderState->expandbuf[ribreaderState->expandidx++] = '\\';
							ribreaderState->expandbuf[ribreaderState->expandidx++] = 'n';
							break;
						case 0x0c:
							ribreaderState->expandbuf[ribreaderState->expandidx++] = '\\';
							ribreaderState->expandbuf[ribreaderState->expandidx++] = 'f';
							break;
						case 0x0d:
							ribreaderState->expandbuf[ribreaderState->expandidx++] = '\\';
							ribreaderState->expandbuf[ribreaderState->expandidx++] = 'r';
							break;
						case 0x7f:
							ribreaderState->expandbuf[ribreaderState->expandidx++] = '\\';
							ribreaderState->expandbuf[ribreaderState->expandidx++] = 'b';
							break;
						case '"':
							ribreaderState->expandbuf[ribreaderState->expandidx++] = '\\';
							ribreaderState->expandbuf[ribreaderState->expandidx++] = '"';
						...
						default:
							if (c<32)
								ribreaderState->expandidx += sprintf(&ribreaderState->expandbuf[ribreaderState->expandidx],"\\%o",c);
							else
								ribreaderState->expandbuf[ribreaderState->expandidx++] = c;
					}
			}
			...
			union {
				unsigned char t[4];
				float f;		// Required to be in 4-byte IEEE format
			} u;
#if BYTE_ORDER == BIG_ENDIAN
			u.t[0] = ribreadGetByte();
			u.t[1] = ribreadGetByte();
			u.t[2] = ribreadGetByte();
			u.t[3] = ribreadGetByte();
#else
			u.t[3] = ribreadGetByte();
			u.t[2] = ribreadGetByte();
			u.t[1] = ribreadGetByte();
...
#endif
			ribreadRequireExpandbuf(20);	// Max size of decimal expansion of a number + padding
			ribreaderState->expandidx += sprintf(&ribreaderState->expandbuf[ribreaderState->expandidx],"%g ",u.f);
			...
			union {
				unsigned char t[8];
				double d;			// Required to be in 8-byte IEEE format
			} u;
#if BYTE_ORDER == BIG_ENDIAN
			u.t[7] = ribreadGetByte();
			u.t[6] = ribreadGetByte();
			u.t[5] = ribreadGetByte();
			u.t[4] = ribreadGetByte();
			u.t[3] = ribreadGetByte();
			u.t[2] = ribreadGetByte();
			u.t[1] = ribreadGetByte();
			u.t[0] = ribreadGetByte();
			...
			ribreadRequireExpandbuf(20);	// Max size of decimal expansion of a number + padding
			if (u.d == (int)u.d)
				ribreaderState->expandidx += sprintf(&ribreaderState->expandbuf[ribreaderState->expandidx],"%d ",(int)u.d);
			else
				ribreaderState->expandidx += sprintf(&ribreaderState->expandbuf[ribreaderState->expandidx],"%Lf ",u.d);


		} else if (c == 0246) {
			// Reference RI request
			const int code = ribreadGetByte();

			if (!ribreaderState->ri_request[code])
				// No stored RI request
				fatal(CODE_BADTOKEN,"Binary encoded RIB corrupt\n");

			const int len = strlen(ribreaderState->ri_request[code]);
			ribreadRequireExpandbuf(len+2);
			ribreaderState->expandbuf[ribreaderState->expandidx++] = 0x0a;	// Insert newline, since all RI requests should begin on a new line
			strcpy(&ribreaderState->expandbuf[ribreaderState->expandidx],ribreaderState->ri_request[code]);
			ribreaderState->expandidx += len;
			ribreaderState->expandbuf[ribreaderState->expandidx++] = ' ';	// Replace '\0' with space instead


		} else if (c >= 0310 && c <= 0313) {
			// Floating point array
			const int l = c - 0310 + 1;
			int len = 0;
			for (j=0; j<l; j++)
				len = (len << 8) | ribreadGetByte();
			ribreadRequireExpandbuf(len*20+3);	// Max size of decimal expansion of a number + padding
			ribreaderState->expandbuf[ribreaderState->expandidx++]='[';
			ribreaderState->expandbuf[ribreaderState->expandidx++]=' ';
			for (j=0; j<len; j++) {
				int val = 0;
				...
				u.t[0] = ribreadGetByte();
				ribreaderState->expandidx += sprintf(&ribreaderState->expandbuf[ribreaderState->expandidx],"%g ",u.f);
				assert(ribreaderState->expandidx < ribreaderState->expandsize);
			}
			ribreaderState->expandbuf[ribreaderState->expandidx++]=']';
			...
			// Define RI request
			const int code = ribreadGetByte();
			unsigned char c = ribreadGetByte();
			if (c<128) {
				// non-encoded string
				// The spec doesn't specify how this is stored (quoted? zero terminated?) or even if it is allowed, and no
				// tools seem to write RI requests as non-encoded strings  FIXME
				assert(0);
			} else if (c >= 0220 && c <= 0243) {
				// encoded string
				int len;
				...
				if (ribreaderState->ri_request[code])
					free(ribreaderState->ri_request[code]);
				char* p = (char*)malloc(len+1);
				ribreaderState->ri_request[code] = p;
				for(j=0;j<len;j++)
					p[j] = ribreadGetByte();
				ribreaderState->ri_request[code][len] = 0;
			} else {
				// Define string, but encoded definition is not a string
				fatal(CODE_BADTOKEN,"Binary encoded RIB corrupt\n");
			}


		} else if (c >= 0315 && c <= 0316) {
			// String dictionary store
			const int w = c - 0315;
			unsigned int token = ribreadGetByte();
			if (w==1)
				token = (token << 8) | ribreadGetByte();
			unsigned char c = ribreadGetByte();
			if (c<128) {
				if (c != '"')
					// Strings needs to be quoted with '"'
					fatal(CODE_BADTOKEN,"Binary encoded RIB corrupt\n");
				// Search for end-of-string citation mark
				// We have no idea of the length of this, so alloc a temp buffer
				// to store string in while we scan for end-quote
				int tmplen = 128;
				...
				while(1) {
					...
					if (len > tmplen) {
						tmplen += 128;
						tmp = (char*)realloc(tmp, tmplen+1);
					}
				}
				tmp[len] = 0;
				Tstringdef *ref = new Tstringdef;
				ref->token = token;
				ref->string = strdup(tmp);
				ref->length = len;
				ref->next = ribreaderState->stringrefs;
				ribreaderState->stringrefs = ref;
				free(tmp);


			} else if (c >= 0220 && c < 0244) {
				// encoded string
				int len;
				if (c <= 0237) {
					// Short string
				...
				char* tmp = (char*)malloc(4*len+1);		// Worst case; all characters needs octal escapes
				int i=0;
				for(j=0; j<len; j++) {
					char c=ribreadGetByte();
					switch(c) {
						case 0x09:	tmp[i++] = '\\'; tmp[i++] = 't';  break;
						case 0x0a:	tmp[i++] = '\\'; tmp[i++] = 'n';  break;
						case 0x0c:	tmp[i++] = '\\'; tmp[i++] = 'f';  break;
						case 0x0d:	tmp[i++] = '\\'; tmp[i++] = 'r';  break;
						case 0x7f:	tmp[i++] = '\\'; tmp[i++] = 'b';  break;
						case '\\':	tmp[i++] = '\\'; tmp[i++] = '\\'; break;
						case '"':	tmp[i++] = '\\'; tmp[i++] = '"';  break;
						default:
							if (c<32)
								i += sprintf(&tmp[i],"\\%o",c);
				...
				Tstringdef *ref = new Tstringdef;
				ref->token = token;
				ref->string = strdup(tmp);		// Usually ~1/4 length of tmp
				ref->length = i;
				ref->next = ribreaderState->stringrefs;
				ribreaderState->stringrefs = ref;
				free(tmp);
			...
			// String dictionary reference
			const int w = c - 0317;
			int token = ribreadGetByte();
			if (w==1)
				token = (token << 8) | ribreadGetByte();

			// Find string in linked list of defined strings
			Tstringdef *ref = NULL;
			for (ref=ribreaderState->stringrefs;ref->next;ref=ref->next) {
				if (ref->token == token)
					break;
			}
			if (!ref) {
				// Cannot find encoded string in dictionary
				fatal(CODE_BADTOKEN,"Binary encoded RIB corrupt\n");
			}
			ribreadRequireExpandbuf(ref->length+3);
			ribreaderState->expandbuf[ribreaderState->expandidx++] = '"';
			memcpy(&ribreaderState->expandbuf[ribreaderState->expandidx],ref->string,ref->length);
			ribreaderState->expandidx += ref->length;
			ribreaderState->expandbuf[ribreaderState->expandidx++] = '"';
			ribreaderState->expandbuf[ribreaderState->expandidx++] = ' ';
