...
void	COcclusionCuller::resetHierarchy(COcclusionNode *cNode) {
	...
	if (cNode->width > 1) {
		...
		cNode->zmax	=	maxx(	maxx(cNode->children[0]->zmax,cNode->children[1]->zmax),
								maxx(cNode->children[2]->zmax,cNode->children[3]->zmax));
	}
}
...
int COcclusionCuller::probeRect(int *xbound,int *ybound, int bw, int bh, int bl, int bt, float zmin) {
	...
	for(;queryDepth>0;queryDepth--) {
		...
		//			xmax		=	minn(xmax,(1<<queryDepth)-1);
		//			ymax		=	minn(ymax,(1<<queryDepth)-1);
		// 		This is incorrect because it doesn't account for buckets straddling bucket edge
		//			xmax		=	minn(xmax,(bw>>(depth-queryDepth))-1);
		//			ymax		=	minn(ymax,(bh>>(depth-queryDepth))-1);
