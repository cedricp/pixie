...
int							ifexprlex(void );		// Forward definition for stupid yacc
void						ifexprerror(const char *,...);
...
int		CRendererContext::ifParse(const char *expr) {
	...
	newState		=	ifexpr_scan_string(expr);				// Create a new buffer
	ifexprparse();												// Scan the buffer
	ifexpr_delete_buffer(newState);								// Ditch the buffer

	ifexpr_switch_to_buffer( savedState );						// Switch to the old buffer
