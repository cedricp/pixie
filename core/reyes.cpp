...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	#define __logObjectRasterizeDice(__object,__x,__y)		fprintf(stderr,"->object %x rasterized / diced at %d,%d in thread %d\n",__object,__x,__y,thread);
	...
	#define __logBucketStart(__x,_y)						fprintf(stderr,"START bucket %d,%d in thread %d\n",__x,_y,thread);
	...
	void allocateMapCheck() {
		...
		for(std::map<void*,int>::iterator it = allocMap.begin(); it != allocMap.end(); it++) {
	...
	#define __recordObjectDelete(__object)			\
		osLock(debugMutex);							\
		std::map<void*,int>::iterator it = allocMap.find(__object);	\
	...
	for (y=0;y<CRenderer::yBuckets;y++) {
		for (x=0;x<CRenderer::xBuckets;x++) {
			if ((cBucket = buckets[y][x]) != NULL)	{
				...
				if ( buckets[y][x] ) delete buckets[y][x];
			}
		}
		delete [] buckets[y];
	}
...
#define computeExtends																									\
	...
	bucketPixelWidth	=	minn(CRenderer::bucketWidth,		CRenderer::xPixels-bucketPixelLeft);						\
	bucketPixelHeight	=	minn(CRenderer::bucketHeight,	CRenderer::yPixels-bucketPixelTop);							\
	...
	while(TRUE) {
		...
		if (job.type == CRenderer::CJob::TERMINATE) {
			...
			//printf("thread %i receive job x %i y %i\n",thread,x,y);


			if (x >= CRenderer::xBuckets) error(CODE_BUG, "x bucket not in screen boundary\n");
			if (y >= CRenderer::yBuckets) error(CODE_BUG, "y bucket not in screen boundary\n");
...
void	CReyes::render() {
	...
	memBegin(threadMemory);
		...
		// Get the framebuffer
		rasterEnd(pixelBuffer,nullBucket);
...
void	CReyes::skip() {
	...
	__logBucketSkip(currentXBucket,currentYBucket);

	while((cObject = cBucket->objects) != NULL) {
		cBucket->objects	=	cObject->next[thread];
		__logObjectSkip(cObject,currentXBucket,currentYBucket);
		objectDefer(cObject);
	}
...
void		CReyes::drawObject(CObject *object) {
	...
	const float	zmin	=	maxx(bmin[COMP_Z],CRenderer::clipMin);
	const float	zmax	=	minn(bmax[COMP_Z],CRenderer::clipMax);
	...
	if (CRenderer::projection == OPTIONS_PROJECTION_PERSPECTIVE) {
		...
		// Do the perspective divide and figure out the extend on the screen
	...
	if (CRenderer::aperture != 0) {
		const	float	mcoc	=	maxx(cocScreen(zmin),cocScreen(zmax));
		xmin					=	xmin-mcoc;
		xmax					=	xmax+mcoc;
		ymin					=	ymin-mcoc;
		ymax					=	ymax+mcoc;
	...
	xmin							=	maxx(xmin,0);
	ymin							=	maxx(ymin,0);
	xmax							=	minn(xmax,CRenderer::sampleClipRight);
	ymax							=	minn(ymax,CRenderer::sampleClipBottom);
...
void CReyes::addDeepFragment(CRasterGrid *grid)
{

	float **varying	=	currentShadingState->varying;
	int	numVertices	=	(grid->vdiv + 1) * (grid->udiv + 1);

	if (grid->dim == SHADING_0D) {
		numVertices =  grid->vdiv;
	}

	struct CDeepFragment{
		vector P, N, Ng, dpdu, dpdv;
		float du, dv, u, v, s, t;
	};

	for (int i=numVertices;i>0;i--){
		CDeepFragment deepFrag;
		movvv(deepFrag.P, &varying[VARIABLE_P][i*3]);
		movvv(deepFrag.N, &varying[VARIABLE_N][i*3]);
		movvv(deepFrag.Ng, &varying[VARIABLE_NG][i*3]);
		movvv(deepFrag.dpdu, &varying[VARIABLE_DPDU][i*3]);
		movvv(deepFrag.dpdv, &varying[VARIABLE_DPDV][i*3]);
		deepFrag.du = varying[VARIABLE_DU][i];
		deepFrag.dv = varying[VARIABLE_DV][i];
	}
...
void		CReyes::insertGrid(CRasterGrid *grid,int flags) {
	...
	if (CRenderer::aperture != 0) {
		...
		const	float	mcoc	=	maxx(coc1,coc2);
	...
	if (flags & RASTER_POINT) {
		...
		for (i=grid->numVertices;i>0;i--,vertices+=numVertexSamples,sizes+=2,bounds+=4) {
			...
			if (grid->flags & RASTER_MOVING) {
				...
				xbound[0]		=	minn(xbound[0],P[0]);
				xbound[1]		=	maxx(xbound[1],P[0]);
				ybound[0]		=	minn(ybound[0],P[1]);
				ybound[1]		=	maxx(ybound[1],P[1]);

				const float maxSize	=	maxx(sizes[0],sizes[1]);
		...
		for (j=0;j<vdiv;j++) {
			for (i=0;i<udiv;i++,bounds+=4) {
				...
				if (CRenderer::aperture != 0) {
					...
					const float	mcoc	=	maxx(maxx(maxx(c1,c2),c3),c4);
	...
	for (i=0;i<CRenderer::numThreads;i++) {
		...
		if (!killObj && (bx < CRenderer::xBuckets) && (by < CRenderer::yBuckets)) {
			...
			if (cBucket != NULL) {
				...
				__logObjectInsert(object,cx,cy,i,cBucket->queue != NULL);
