...
///////////////////////////////////////////////////////////////////////
// Function				:	readLayerPng
// Description			:	Read a layer of the image into memory
// Return Value			:
// Comments				:
void	*readLayerPng(const char *file,int *width,int *height,int *bitsperpixel,int *numSamples) {
	void* data = NULL;
	uint16 ns = 0,pixelSize;
    png_structp png_ptr;
    png_infop info_ptr;
    ...
    if (in){
        ...
        if (png_ptr){
            ...
            if (info_ptr){
                ...
                data = (void*)malloc(Width*Height*pixelSize);
...
void	makeTexture(const char *input,const char *output,TSearchpath *path,const char *smode,const char *tmode,RtFilterFunc filt,float fwidth,float fheight,int numParams,const char **params,const void **vals) {
	...
	if (CRenderer::locateFile(inputFileName,input,path) == FALSE) {
		...
		if (inHandle == NULL) {
		    ...
		    data		=	readLayerPng(input,&width,&height,&bitspersample,&numSamples);
		    if(data){
