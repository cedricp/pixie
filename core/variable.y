...
void yyerror(const char *);
		...
		void							variableerror(const char *);		// Forward definition for stupid yacc
		int								variablelex(void );				// Forward definition for stupid yacc
...
#include	"lex.variable.cpp"

static	int	numErrors	=	0;

void	variableerror(const char *str) {
...
int	parseVariable(CVariable *var,const char *name,const char *decl) {
	...
	newState	=	variable_scan_string(decl);
	variableparse();
	variable_delete_buffer(newState);

	variable_switch_to_buffer( savedState );
