...
displayControl*                 CRenderer::deepImageWrap            =   NULL;
FILE							*CRenderer::deepShadowFile			=	NULL;						// initialized in beginDisplays
deepImage*						CRenderer::deepImg					=	NULL;						// initialized in beginDisplays
...
char							*CRenderer::deepImageFileName		=	NULL;						// initialized in beginDisplays / computeDisplayData
...
int								CRenderer::origin[2];
int								CRenderer::originalsize[2];
...
static void	copyOptions(const COptions *o) {
	...
	CRenderer::dimgThreshold			=	o->tsmThreshold;
...
void		CRenderer::beginFrame(const COptions *o,CAttributes *a,CXform *x) {
    ...
    origin[0] = xres * cropLeft;
    origin[1] = yres * cropTop;

    originalsize[0] = xres * (cropRight - cropLeft);
    originalsize[1] = yres * (cropBottom - cropTop);
	...
	// The sample offsets
	xSampleOffset		=	(int) ceil(maxx(	(CRenderer::pixelFilterWidth-1)*CRenderer::pixelXsamples  / 2.0 , 0));
	ySampleOffset		=	(int) ceil(maxx(	(CRenderer::pixelFilterHeight-1)*CRenderer::pixelYsamples / 2.0 , 0));
	...
	// Set the root object
	root					=	new CDummyClusterObject(a,x);
...
void		CRenderer::endFrame() {
	...
	// Write out the deepimage buffer
	if (deepImg){
		deepImg->write();
		delete deepImg;
		deepImg = NULL;
