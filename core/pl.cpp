...
// Return Value			:
// Comments				:
void	CPl::collect(int &size,float *&data,EVariableClass container,CMemPage *page,bool useMalloc) {
	int			i,j,k;
	int			vs			=	0;
	int			numItems	=	0;
	...
	for (i=0;i<numParameters;i++) {
		if (parameters[i].container == container) {
			...
			char *rmr = parameters[i].variable->name;
		}
	}
	...
	if (data == NULL)	{
		...
		if(useMalloc)
            data	=	(float *) malloc(size*numItems*sizeof(float));
        else
