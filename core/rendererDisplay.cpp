...
// FIXME: depth quantizer for z
	...
	if (strcmp(name,"quantize") == 0) {
		...
		if ((numItems == 2) && (type == INTEGER_PARAMETER)){
            return	CRenderer::origin;
		}
	} else if (strcmp(name,"OriginalSize") == 0) {
		if ((numItems == 2) && (type == INTEGER_PARAMETER)){
            return	CRenderer::originalsize;
		}
...
void	CRenderer::endDisplays() {
	...
	for (i=0;i<numDisplays;i++) {
		...
		if (datas[i].module != NULL) {
		    displayControl* dspCtl = static_cast<displayControl*>(datas[i].cppDisplayWrap);

		    if(dspCtl){
		        dspCtl->displayFinish( datas[i].handle );
		    } else {
...
void	CRenderer::dispatch(int left,int top,int width,int height,float *pixels) {
	...
	for (i=0;i<numDisplays;i++) {
		if (datas[i].module != NULL) {
			...
			if ( dspCtl && !deepImageWrap && ( dspCtl->displayData(datas[i].handle,left,top,width,height,dispatchData) == FALSE) ){
			...
			if ( datas[i].data && (datas[i].data(datas[i].handle,left,top,width,height,dispatchData) == FALSE) ) {
				// Lock this piece of code
				osLock(displayKillMutex);
				datas[i].handle	=	NULL;
				numActiveDisplays--;
				...
				if ( ! datas[i].customDisplay ) osUnloadModule(datas[i].module);
...
void	CRenderer::computeDisplayData() {
	...
	for (cDisplay=displays;cDisplay!=NULL;cDisplay=cDisplay->next) {
		...
		datas[numDisplays].cppDisplayWrap  =	NULL;
		datas[numDisplays].customDisplay = false;
		...
		if (strcmp(outDevice,"deepimage") == 0) {
			...
			if (strcmp(hider,"stochastic") != 0) {
				error(CODE_LIMIT,"Hider must be stochastic or hidden for deep image\n");
				deepImg		= NULL;
			} else if (deepImg != NULL) {
				error(CODE_LIMIT,"There can only be one deep image output\n");
			} else {
				if (netClient != INVALID_SOCKET) {
					deepImg		= NULL;
					error(CODE_SYSTEM,"You can't net-render a deep image\n");
				} else {
					deepImg		=	new deepImage;
				}
				...
				if (deepImg != NULL) {
					...
					deepImageFileName			=	strdup(displayName);
		...
		} else	if (strcmp(outDevice,"tsm") == 0) {
			int					j;
			...
			if (strcmp(outDevice,"custom_c") == 0) {
				...
				datas[numDisplays].cppDisplayWrap=   NULL;
				datas[numDisplays].customDisplay =  true;
			} else if (strcmp(outDevice,"custom_cpp") == 0) {
				datas[numDisplays].module		=	(void *) outDevice;
				datas[numDisplays].start		=	NULL;
				datas[numDisplays].data			=	NULL;
				datas[numDisplays].rawData		=	NULL;
				datas[numDisplays].finish		=	NULL;
				datas[numDisplays].cppDisplayWrap=  static_cast<displayControl*>(cDisplay->cppDisplayWrap);
				datas[numDisplays].customDisplay =  true;
			} else if (strcmp(outDevice,"custom_deepimage") == 0) {
                /*if ((1 << header.tileShift) != bucketWidth) {
                    error(CODE_LIMIT,"Bucket width must be a power of 2 for DEEPIMAGE: %d\n",bucketWidth);
                } else  {*/
                    if (bucketWidth != bucketHeight) {
                        error(CODE_LIMIT,"Bucket width and height must be same for DEEPIMAGE: (%d,%d)\n",bucketWidth,bucketHeight);
                    } else {
                        if (strcmp(hider,"stochastic") != 0) {
                            error(CODE_LIMIT,"Hider must be stochastic or hidden for DEEPIMAGE\n");
                        } else {
                            if (deepImageWrap != NULL) {
                                error(CODE_LIMIT,"There can only be one DEEPIMAGE output\n");
                            } else {
                                datas[numDisplays].module		=	(void *) outDevice;
                                datas[numDisplays].start		=	NULL;
                                datas[numDisplays].data			=	NULL;
                                ...
                                datas[numDisplays].cppDisplayWrap=  static_cast<displayControl*>(cDisplay->cppDisplayWrap);
                                datas[numDisplays].customDisplay =  true;
                                deepImageWrap = static_cast<displayControl*>(cDisplay->cppDisplayWrap);
                                flags |=	OPTIONS_FLAGS_DEEP_IMAGE_RENDERING;
                            }
                        }
                    }
			...
			// Check the stuff
			bool cfuncok   = datas[numDisplays].start != NULL && datas[numDisplays].data != NULL && datas[numDisplays].finish != NULL;
			bool cppfuncok = datas[numDisplays].cppDisplayWrap != NULL;

			if (  !cfuncok && !cppfuncok ) {
				...
				displayControl* dspCtl      =   static_cast<displayControl*>(datas[numDisplays].cppDisplayWrap);

				if (dspCtl){
				    datas[numDisplays].handle	=	dspCtl->displayStart(displayName,xres,yres,datas[numDisplays].numSamples,cDisplay->outSamples,findParameter);
				} else {
