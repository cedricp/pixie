...
void        CRibOut::RiPtFilterV(const char *filterType, const char* filename, const char* outfilename, int n, const char *tokens[], const void* params[]){
	out("PtFilter \"%s\" \"%s\" \"%s\" \n",filterType,filename,outfilename);
...
void		CRibOut::RiAbcArchiveV(const char *name,int n,const char *tokens[],const void *params[]) {
	out("AbcArchive \"%s\" ",name);
	writePL(n,tokens,params);
}
...
void		CRibOut::RiCustomDisplay2V(const char *name, RtToken mode, void* cppDisplayWrap, RtInt n, RtToken tokens[], RtPointer params[]) {
	error(CODE_INCAPABLE,"Can not serialize custom displays.\n");
}
...
void		CRibOut::RiOptionV(const char *name,int n,const char *tokens[],const void *params[]) {
	...
	// Check for rib compression / output options
	    ...
	    char ptype [20];
	    char pname [30];
	    sscanf(tokens[0],"%s %s",ptype,pname);
	    if(strcmp(ptype,"float") == 0){
            const float	*val	=	(const float *) params[0];
            out("Option \"%s\" \"%s\" [ %g ]\n",name,tokens[0],val[0]);
	    } else {
            const char	*val	=	((const char **) params[0])[0];
            out("Option \"%s\" \"%s\" \"%s\"\n",name,tokens[0],val);
	    }
	} else {
...
void		CRibOut::RiPointsGeneralPolygonsV(int npolys,int *nloops,int *nverts,int *verts,int n,const char *tokens[],const void *params[]) {
	...
	out("[");
	for (i=0;i<sverts;i++) {
		nvertices	=	maxx(nvertices,verts[i]+1);
		out("%d ",verts[i]);
	}
	out("] ");
...
void		CRibOut::declareDefaultVariables() {
    ...
    declareVariable("albedo",		    "color");
    declareVariable("dmfp",			    "color");
    declareVariable("ior",			    "float");
    declareVariable("maxsolidangle",    "float");
    declareVariable("unitlength",	    "float");
