...
void			CSphere::computeObjectBound(float *bmin,float *bmax,float r,float vmin,float vmax,float umax) {
	float	maxRadius;
	float	vmi		=	minn(vmin,vmax);
	float	vma		=	maxx(vmin,vmax);
	float	zmin;
	float	zmax;
	...
	initv(bmin,-maxRadius,-maxRadius,minn(zmin,zmax));
	initv(bmax,maxRadius,maxRadius,maxx(zmin,zmax));
}
...
void			CCone::computeObjectBound(float *bmin,float *bmax,float r,float height,float umax) {
	...
	initv(bmin,-r,-r,minn(height,0));
	initv(bmax,r,r,maxx(height,0));
}
...
void			CParaboloid::computeObjectBound(float *bmin,float *bmax,float r,float zmin,float zmax,float umax) {
	...
	initv(bmin,-r,-r,minn(zmin,zmax));
	initv(bmax,r,r,maxx(zmin,zmax));
}
...
void			CHyperboloid::computeObjectBound(float *bmin,float *bmax,float *p1,float *p2,float umax) {
	...
	bmin[COMP_X]	=	-d;
	bmin[COMP_Y]	=	-d;
	bmin[COMP_Z]	=	minn(p1[COMP_Z],p2[COMP_Z]);
	bmax[COMP_X]	=	+d;
	bmax[COMP_Y]	=	+d;
	bmax[COMP_Z]	=	maxx(p1[COMP_Z],p2[COMP_Z]);
}
