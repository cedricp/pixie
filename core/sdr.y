												...
												if (reference) {
													...
													for (tLabel=currentData.labelDefinitions;tLabel!=NULL;tLabel=tLabel->next) {
														if (strcmp(tLabel->name,cLabel->name) == 0) {
															sdrerror("Duplicate label definition\n");
														}
													}
				...
				slOperandList
				{
					...
					case 2:
						...
						if ((dso = CRenderer::getDSO($2,$5)) != NULL) {
							...
							sdrerror("Failed to locate DSO function\n");
						}
						break;
					default:
						break;
...
CShader	*parseShader(const char *shaderName,const char *name) {
	...
	sdr_switch_to_buffer(sdr_create_buffer( fin, YY_BUF_SIZE ) );
	...
	if (currentData.numErrors != 0) {
		sdr_delete_buffer( YY_CURRENT_BUFFER );
		fclose(fin);
		sdr_switch_to_buffer( oldState );
		return NULL;
	}
