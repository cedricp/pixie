...
void	CRaytracer::renderingLoop() {
	...
	while(TRUE) {
		...
		if (job.type == CRenderer::CJob::TERMINATE) {
			...
			width			=	minn(CRenderer::bucketWidth,CRenderer::xPixels-left);
			height			=	minn(CRenderer::bucketHeight,CRenderer::yPixels-top);
	...
	// Generate the image
	{
		...
		for (j=0;j<ysamples;j+=8) {
			for (i=0;i<xsamples;i+=8) {
				const int	my	=	minn(ysamples-j,8);
				const int	mx	=	minn(xsamples-i,8);
...
void	CRaytracer::splatSamples(CPrimaryRay *samples,int numShading,int left,int top,int xpixels,int ypixels) {
	...
	CPrimaryRay *raySamples = samples;
	// Process each sample
	for (int i=0;i<numShading;i++,samples++) {
		...
		pl				=	maxx(pl,left);
		pt				=	maxx(pt,top);
		pr				=	minn(pr,left + xpixels - 1);
		pb				=	minn(pb,top + ypixels - 1);
		float	cx,cy;

		std::vector<deepPixel> deeps;
		deeps.clear();
