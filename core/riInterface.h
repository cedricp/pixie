...
public:
    ...
    virtual	void		RiCustomDisplay2V (const char *name, RtToken mode, void* cppDisplayWrap, RtInt n, RtToken tokens[], RtPointer params[]);
	...
	virtual void		RiAbcArchiveV(const char* name, int n,const char *tokens[],const void *params[]);
    ...
    virtual	void		RiMakeSphericalEnvironmentV(const char *pic,const char *tex,float (*filterfunc)(float,float,float,float),float swidth,float twidth,int n,const char *tokens[],const void *params[]);
	...
	virtual	void		RiPtFilterV(const char *filterType, const char* filename, const char* outfilename, int n, const char *tokens[], const void* params[]);
