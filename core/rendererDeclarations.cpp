...
void			CRenderer::initDeclarations() {
	...
	declareVariable("origin",				"int[2]");
	declareVariable("OriginalSize", 		"int[2]");
	...
	declareVariable(RI_TRACE,				"int");	// Obsolete, but added so we can give a warning
	...
	declareVariable(RI_ABCARGS,				"string");
	declareVariable("objectpath",			"string");
	declareVariable("frame",				"int");
	declareVariable("flipv",				"int");
	declareVariable("fps",					"int");
	...
	tmp	=	declareVariable("otime","global uniform float",PARAMETER_DTIME);	 tmp->entry			=	VARIABLE_OTIME;
