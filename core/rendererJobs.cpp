...
void			CRenderer::dispatchReyes(int thread,CJob &job) {
	...
	if (hiderFlags & (HIDER_DONE | HIDER_BREAK)) {
		...
		while(TRUE) {
			...
			// Has this bucket been assigned to someone ?
            if (jobAssignment[(y*xBuckets)+x] == -1) {
				int	i;

				// Nop, allocate the next stride of buckets to this thread
				for (i=0;i<threadStride;i++) {
				    int currentpos = y*xBuckets+x+i;
					if ((currentpos) < (xBuckets*yBuckets)){
                            jobAssignment[currentpos] = thread;
                            //printf("Assigned %i to %i\n",currentpos,thread);
					}
				}
		...
		if (y < yBuckets) {
		    if(x >= xBuckets) error(CODE_BUG,"X bucket outside screen\n");

			job.type	=	CJob::BUCKET;
			job.xBucket	=	x;
			job.yBucket	=	y;
			// tag as assigned job
			jobAssignment[y*xBuckets+x] = -2;
		} else {
			job.type	=	CJob::TERMINATE;
			numActiveThreads--;
		}
...
void			CRenderer::dispatchPhoton(int thread,CJob &job) {
	...
	if (currentPhoton < numEmitPhotons) {
		...
		job.numPhotons	=	minn(1000,numEmitPhotons-currentPhoton);	// Shoot 1000 photons at a time
...
#define	bucket(__x,__y)		jobAssignment[__y*xBuckets + __x]
	...
	while(TRUE) {
		...
		if (bucket(x,y) == -1) {
			...
			int	right	=	minn((left + netXBuckets),xBuckets);
			int	top		=	(y / netYBuckets)*netYBuckets;
			int	bottom	=	minn((top + netYBuckets),yBuckets);
			int	i,j;
