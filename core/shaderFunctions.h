...
#define PHONGEXPR				normalizev(Ltmp,L);													\
								const float coefficient = (1.0f-ns[0]) * (float) pow(maxx(0,dotvv(refDir,Ltmp)),*size);			\
								if (coefficient > 0) {												\
									R[COMP_R] += coefficient * Cl[COMP_R];							\
									R[COMP_G] += coefficient * Cl[COMP_G];							\
									R[COMP_B] += coefficient * Cl[COMP_B];							\
...
#define SPECULARBRDFEXPR		addvv(halfway,op3,op1);												\
								normalizev(halfway);												\
								res[0]	=	(float) pow(maxx(0,dotvv(op2,halfway)),(10.0f)/(*op4));	\
								res[1]	=	res[0];													\
								res[2]	=	res[1];													\
...
#define LIGHTPARAMETEREXPR_PRE	float		*res;															\
								...
								if(*currentLight != NULL) \
                                    ...
                                    error(CODE_ILLSTATE,"lightsource call not in illumination loop\n");\
								const float	*src		=	op2;											\
								int			srcStep		=	op2Step;										\
																											\
								if (found > 0.0) {															\
...
#define	TEXTUREFEXPR			plReady();																		\
                                ...
                                if ( scratch->textureParams.linearize > 0 ) res[i] = linearize( tmp[int(*op2)&3] ); \
								else res[i]		=	tmp[int(*op2)&3];
...
#define	TEXTURECEXPR			plReady();																		\
								...
								if ( scratch->textureParams.linearize > 0 ) {for (int k = 0; k < 3; k++ ) res[k] = linearize(res[k]);}
...
#define	ENVIRONMENTEXPR_PRE(__name)																					\
								...
								CTraceLocation	*rays = NULL;														\
								int				numRays = 0;														\
								const float		*P = NULL,*N = NULL;												\
								float			*dPdu = NULL,*dPdv = NULL;											\
								if (tex == NULL) {																	\
...
#define	ENVIRONMENTEXPR(__float)																					\
								...
								if (tex == NULL) {																	\
									...
									rays->coneAngle		=	maxx(scratch->traceParams.coneAngle,scratch->textureParams.blur);	\
...
#define	FILTERSTEP2EXPR_PRE		FUN3EXPR_PRE																		\
								...
								for (int i=0;i<numVertices;++i) {													\
									...
									fwidth[i]	=	scratch->textureParams.width*maxx(dsdu[i] + dsdv[i],C_EPSILON);	\
								}
...
#define	BAKE3DEXPR				plReady();																		\
								...
								if (scratch->texture3dParams.radius > 0) {										\
									...
									radius	=	(lengthv(dPdu) + lengthv(dPdv))*0.25*scratch->texture3dParams.radiusScale;		\
								}																				\
																												\
...
// volume3d	"v=Sfpf"
#ifndef INIT_SHADING
#define	VOLUME3DEXPR_PRE 		VdbReader* reader = VdbReader::getVdbReader();									\
								float	    *res, *opcol, color[3];                                                       \
								const float *op2, *op3, *op4;            		        						\
								const char **op1;       													    \
								operand(0,res,float *);	                                                        \
								operand(1,op1,const char **);	                                                \
								operand(2,op2,const float *);                                               	\
...
#define	VOLUME3DEXPR			reader->getElement(res, *op1, (int)*op2, op3, *op4) == true ? 1.0 : 0.0;       \


#define	VOLUME3DEXPR_UPDATE	    res += 3;											    								\
								op3 += 3;                                                                           \

#define	VOLUME3DEXPR_POST
#else
#define	VOLUME3DEXPR_PRE
#define	VOLUME3DEXPR
#define	VOLUME3DEXPR_UPDATE
#define	VOLUME3DEXPR_POST
#endif

DEFFUNC(Volume3d			,"voxel3d"					,"v=Sfpf"		,VOLUME3DEXPR_PRE,VOLUME3DEXPR,VOLUME3DEXPR_UPDATE,VOLUME3DEXPR_POST,0)

#undef	VOLUME3DEXPR_PRE
#undef	VOLUME3DEXPR
#undef	VOLUME3DEXPR_UPDATE
#undef	VOLUME3DEXPR_POST

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// volumeinfo	"f=SfSv"
#ifndef INIT_SHADING
#define	VOLUMEINFOEXPR_PRE 		VdbReader* reader = VdbReader::getVdbReader();									\
								float				*res;                                                       \
								const float *op2;            		        						            \
								float *op4;                                                                     \
								const char **op1, **op3;       													    \
								...
								operand(1,op1,const char **);	                                                \
								operand(2,op2,const float *);                                               	\
								operand(3,op3,const char **);	                                                \
								operand(4,op4,float *);                                                   \

								

#define	VOLUMEINFOEXPR			if(strcmp(*op3, "size") == 0){                                                  \
                                    *res = (reader->getSize(*op1, (int)*op2, op4) == true) ? 1.0 : 0.0;         \
                                } \


#define	VOLUMEINFOEXPR_UPDATE	    ++res;									    								\

#define	VOLUMEINFOEXPR_POST
#else
#define	VOLUMEINFOEXPR_PRE
#define	VOLUMEINFOEXPR
#define	VOLUMEINFOEXPR_UPDATE
#define	VOLUMEINFOEXPR_POST
#endif

DEFFUNC(VolumeInfo			,"voxelinfo"					,"f=SfSv"   ,VOLUMEINFOEXPR_PRE,VOLUMEINFOEXPR,VOLUMEINFOEXPR_UPDATE,VOLUMEINFOEXPR_POST,0)

#undef	VOLUMEINFOEXPR_PRE
#undef	VOLUMEINFOEXPR
#undef	VOLUMEINFOEXPR_UPDATE
#undef	VOLUMEINFOEXPR_POST
