...
static inline float	minCocPixels(float z1, float z2) {
	return minn(cocPixels(z1),cocPixels(z2));
}
...
//FIXME: implies if either end is culled we cull - wrong
			...
			// Expand the bound
			maxBound		=	maxx(bmax[COMP_X]-bmin[COMP_X],bmax[COMP_Y]-bmin[COMP_Y]);
			maxBound		=	maxx(bmax[COMP_Z]-bmin[COMP_Z],maxBound);
			...
			udiv = maxx(minDivU,udiv);
			vdiv = maxx(minDivV,vdiv);
...
CTesselationPatch::CPurgableTesselation*		CTesselationPatch::tesselate(CShadingContext *context,char rdiv,int estimateOnly) {
		...
		if (!(flags & OBJECT_MOVING_TESSELATION)) {
			...
			if (rdiv == 1) {
				...
				for(int i=0;i<nb*nb;i++){
					float maxBound	=	maxx(bnds[3+COMP_X]-bnds[COMP_X],bnds[3+COMP_Y]-bnds[COMP_Y]);
					maxBound		=	maxx(bnds[3+COMP_Z]-bnds[COMP_Z],maxBound);
	...
	if (rdiv == 1) {
		...
		rmax							=	maxx(rmax,(uAvg+vAvg)/4.0f);
	} else {
		rmax							=	maxx(rmax,div*(uAvg+vAvg)/2.0f);
	}
	...
	// Expand the bound
	float maxBound	=	maxx(bmax[COMP_X]-bmin[COMP_X],bmax[COMP_Y]-bmin[COMP_Y]);
	maxBound		=	maxx(bmax[COMP_Z]-bmin[COMP_Z],maxBound);
	maxBound		*=	boundExpander;
