...
CStochastic::CStochastic(int thread) : CReyes(thread), COcclusionCuller(), apertureGenerator(CRenderer::frame) {
	...
	initCuller(maxx(totalWidth,totalHeight), &maxDepth);
}
...
void		CStochastic::rasterBegin(int w,int h,int l,int t,int nullBucket) {
	...
	// Cedric's hack
	if (!(CRenderer::flags & OPTIONS_FLAGS_DEEP_SHADOW_RENDERING || CRenderer::flags & OPTIONS_FLAGS_DEEP_IMAGE_RENDERING) && nullBucket){
	    isBlank = true;
	    return;
    }
...
void		CStochastic::rasterEnd(float *fb2,int noObjects) {
	...
	else if (CRenderer::flags & OPTIONS_FLAGS_DEEP_IMAGE_RENDERING)	deepImageCompute();
	if (isBlank) {
		// early-out if we have no data
	...
	const int		pixelSize				=	6	+ CRenderer::numExtraSamples;	// (f)alpha + (f)depth + (fff)color + (f)opacity + extra samples
	...
	for (int y=0;y<sampleHeight;y++) {
		...
		for (i=sampleWidth;i>0;i--,cPixel++,cFb+=pixelSize) {
			...
			// cPixel->last is really used, but cPixel->first is not (it's always skipped in the composite),
			...
			//    combined with z-threshold is very awkward.
			...
			for (sy=0;sy<CRenderer::pixelYsamples;sy++) {
				...
				for (i=0;i<xres;i++) {
					for (sx=0;sx<CRenderer::pixelXsamples;sx++,cSample+=pixelSize) {
						cPixel[4]		=	minn(cPixel[4],cSample[1]);
					}
	...
	case DEPTH_MAX:
		...
		for (int y=0;y<yres;y++) {
			...
			for (sy=0;sy<CRenderer::pixelYsamples;sy++) {
				...
				for (i=0;i<xres;i++) {
					for (sx=0;sx<CRenderer::pixelXsamples;sx++,cSample+=pixelSize) {
						cPixel[4]		=	maxx(cPixel[4],cSample[1]);
					}
					cPixel += CRenderer::numSamples;
				}
				//cSample += CRenderer::xSampleOffset*pixelSize;
...
inline	void	outSample(float cZ,const float *opacity,CTSMData &data) {
	...
	if (data.origin[0] == -C_INFINITY) {
		...
		crSlopeMax				=	minn(crSlopeMax,data.rSlopeMax);
		cgSlopeMax				=	minn(cgSlopeMax,data.gSlopeMax);
		cbSlopeMax				=	minn(cbSlopeMax,data.bSlopeMax);

		crSlopeMin				=	maxx(crSlopeMin,data.rSlopeMin);
		cgSlopeMin				=	maxx(cgSlopeMin,data.gSlopeMin);
		cbSlopeMin				=	maxx(cbSlopeMin,data.bSlopeMin);
...
void			CStochastic::filterSamples(int numSamples,CFragment **samples,float *weights) {
	...
	while(TRUE) {
		...
		newOpacity[0]				=	oldOpacity[1]*(1.f-cSample->opacity[0]);
		newOpacity[1]				=	oldOpacity[2]*(1.f-cSample->opacity[1]);
		newOpacity[2]				=	oldOpacity[3]*(1.f-cSample->opacity[2]);
			...
			if ((x < xres) && (y < yres)) {
				...
				for (i=0,sy=0;sy<filterHeight;sy++) {
					for (sx=0;sx<filterWidth;sx++,i++) {
						...
						const float		cx		=	((float)sx + pixels->jx - (float)filterWidth*0.5f*invPixelXsamples);
						const float		cy		=	((float)sy + pixels->jy - (float)filterHeight*0.5f*invPixelYsamples);
...
///////////////////////////////////////////////////////////////////////
// Class				:	CStochastic
// Method				:	deepImageCompute
// Description			:	Compute/write deep image image data
// Return Value			:	-
// Comments				:
void		CStochastic::deepImageCompute() {
    bool        renderToFile        =   CRenderer::deepImageWrap == NULL;
	int			i;
	const int	xres				=	width;
	const int	yres				=	height;
	const int	filterWidth			=	CRenderer::pixelXsamples + 2*CRenderer::xSampleOffset;
	...
	samples			=	(CFragment **)	ralloc(totalHeight*totalWidth*sizeof(CFragment*)	,threadMemory);
	fSamples		=	(CFragment **)	ralloc(filterWidth*filterHeight*sizeof(CFragment*)	,threadMemory);
	fWeights		=	(float *)		ralloc(filterWidth*filterHeight*sizeof(float)		,threadMemory);
	...
	assert(CRenderer::bucketWidth	>= xres);
	assert(CRenderer::bucketHeight	>= yres);
	std::vector<deepPixel> deepPixels;
			...
			if ((x < xres) && (y < yres)) {
				...
				for (i=0,sy=0;sy<filterHeight;sy++) {
					for (sx=0;sx<filterWidth;sx++,i++) {
						...
						const int		ysample	=	y*CRenderer::pixelYsamples + sy;
						fSamples[i]				=	samples[ysample*sampleWidth+xsample];
						fWeights[i]				=	CRenderer::pixelFilterKernel[sy*filterWidth + sx];
					}
				}
				// At this point we've got all samples in the filter area
				...
				if (CRenderer::deepImageWrap){
					// Send data to an eventual display driver
					osLock(CRenderer::deepImageMutex);
					float* data = (float*)alloca( ( sizeof(float) * deepPixels.size() * 6 ) );
					float* data_it = data;

					*(data_it++) = deepPixels.size();

					for ( std::vector<deepPixel>::iterator it = deepPixels.begin(); it != deepPixels.end(); it++){
						memcpy( data_it, &(*it), sizeof(float) * 5 );
						data_it += 5;
					}
					CRenderer::deepImageWrap->displayData( NULL, currentXBucket * width, currentXBucket * height, width, height, data );
					osUnlock(CRenderer::deepImageMutex);
				}
				deepPixels.clear();
