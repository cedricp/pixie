...
public:
    ...
    virtual void *displayStart(const char *name,int width,int height,int numSamples,const char *samples,TDisplayParameterFunction) = 0;
    virtual int  displayData(void *im,int x,int y,int w,int h,float *data)= 0;
    virtual void displayFinish(void *) = 0;
};
#endif
