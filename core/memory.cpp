...
void            resetStatics(){
    allocatedZoneMemory		=	0;
    freedZoneMemory			=	0;
    allocatedPages			=	0;
    freedPages				=	0;
    memoryPageSize			=	ZONE_BASE_SIZE;
    lastPagingTime			=	0;
}
...
*/
	...
	size					=	size > memoryPageSize ? size : memoryPageSize;
	size					=	(size + 7) & ((int)(~7));
	...
	if(!newPage->base){
	    error(CODE_LIMIT,"Out of memory, exiting\n");
	    exit(ENOMEM);
	}
...
// Description			:	Dealocate memory page
// Return Value			:
// Comments				:
