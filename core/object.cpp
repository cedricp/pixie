...
static	float	getDisp(const float *mat,float disp) {
	...
	for (i=0;i<10;i++) {
		...
		alpha	=	maxx(absf(tmp2[0]),absf(tmp2[1]));
		alpha	=	maxx(alpha,absf(tmp2[2]));
		alpha	=	maxx(alpha,absf(tmp2[3]));
...
void		CObject::cluster(CShadingContext *context) {
	...
	front	=	new CDummyClusterObject(attributes,xform);
	back	=	new CDummyClusterObject(attributes,xform);
...
void			   CObject::estimateDicing(float *P,int udiv,int vdiv,int &nudiv,int &nvdiv,float shadingRate,int nonrasterorient) {
   ...
   if (!nonrasterorient) {
	   ...
	   for (j=(vdiv+1);j>0;--j) {
		   ...
		   cP  +=  3;
		   uMax	=	maxx(uMax,total);
		   uMin	=	minn(uMin,total);
	   }
	   ...
	   for (i=(udiv+1);i>0;--i,cP+=3) {
		   ...
		   vMax	=	maxx(vMax,total);
		   vMin	=	minn(vMin,total);
	   }
	} else {	// non raster oriented
	   vector tmp;

	   float maxDim = maxx(CRenderer::dPixeldx,CRenderer::dPixeldy);
   ...
   udivf	=   maxx(1,udivf);
   vdivf	=   maxx(1,vdivf);
   udivf	=	minn(10000,udivf);
   vdivf	=	minn(10000,vdivf);
...
// Return Value			:
// Comments				:
CDummyClusterObject::CDummyClusterObject(CAttributes *a,CXform *x) : CObject(a,x) {
	flags	|=	OBJECT_DUMMY;
}
...
// Return Value			:
// Comments				:
CDummyClusterObject::~CDummyClusterObject() {
}
...
// Return Value			:
// Comments				:
void			CDummyClusterObject::intersect(CShadingContext *,CRay *) {
...
void				CSurface::dice(CShadingContext *rasterizer) {
	...
	int minU,minV;
	int minSplits = maxx(attributes->minSplits,getDicingStats(0,minU,minV));
