...
const	char	*envsphereArgument			=	"-envsphere";
...
int main(int argc, char* argv[]) {
	...
	for (i=1;i<argc;i++) {
		if (strcmp(argv[i],"--help") == 0) {
			...
			textureMode	=	"envlat";
		} else if (strcmp(argv[i],envsphereArgument) == 0) {
			textureMode	=	"envsphere";
	...
	}else if (strcmp(textureMode,"envsphere") == 0) {
		if (currentFile == 2) {
			RiBegin(RI_NULL);
			tokens[currentParameter]	=	"resize";
			vals[currentParameter++]	=	(RtPointer) &resizeMode;
			RiMakeSphericalEnvironmentV(files[0],files[1],filter,filterWidth,filterHeight,currentParameter,tokens,vals);
			RiEnd();
